...
- `targets` (optional): A pipe-separated `|` list of strings for actions that require a target. The player's target must match one of these. E.g., `"outlet|oil|dead fish"`.
...
```json
"when": { "actionId": "investigate", "targets": "upstream factory" },
"do": [
