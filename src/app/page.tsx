...
import { BookOpen, FolderOpen, User, Edit, Trash2, PlusCircle, ArrowLeft, Wrench, Languages, Book, Smartphone } from 'lucide-react';
...
export default function GameSelectionPage() {
  ...
  const handleStartGame = (rulesId: string, version: 'v1' | 'v2' | 'mobile' = 'v1') => {
      if (!activePlayer) return;
      sessionStorage.setItem(PLAYER_STATS_TO_LOAD_KEY, JSON.stringify(activePlayer));
      let path = `/play/${rulesId}`;
      if (version === 'v2') {
        path = `/play_v2/${rulesId}`;
      } else if (version === 'mobile') {
        path = `/play_mobile/${rulesId}`;
      }
      router.push(path);
  };
        ...
        {availableRules.map(rules => (
          <Card key={rules.id} className="flex flex-col">
            ...
            <CardFooter className="flex-col items-stretch gap-2">
              ...
              <Button onClick={() => handleStartGame(rules.id, 'mobile')} className="w-full" variant="outline">
