// src/components/game/mobile/MobileTalkScreen.tsx
'use client';
...
interface MobileTalkScreenProps {
  gameState: GameState;
...
export function MobileTalkScreen({
  gameState,
  characterProfile,
  objective,
  conversationType,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const result = await conversationFlow({
        ...
        objective,
        sceneDescription: 'A tense conversation.', // Placeholder
    ...
    <>
      ...
      <div className="flex flex-col h-full">
        <div className="relative w-full h-1/4 shrink-0">
            ...
            />
             <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-background to-transparent">
                <h2 className="text-2xl font-headline font-bold text-white shadow-lg">{characterProfile.name}</h2>
                <p className="text-sm text-white/90 italic shadow-lg">"{characterProfile.personality}"</p>
             </div>
        </div>
             ...
             <ScrollArea className="h-full pr-4" ref={scrollAreaRef}>
                 <div className="space-y-6">
                    ...
                    <div key={entry.id}
                        ...
                        <div
                        className={`max-w-xs rounded-xl px-4 py-3 ${entry.type === 'player' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                        <p className="whitespace-pre-wrap">{entry.message}</p>
                        </div>
                        {entry.type === 'player' && (
                        <div
                            className="flex-shrink-0 w-8 h-8 rounded-full bg-primary/80 flex items-center justify-center">
        ...
        <div className="p-4 border-t bg-background shrink-0 space-y-4">
            ...
            <Button variant="outline" onClick={() => onConversationEnd(conversation, false)} disabled={objectiveAchieved} className="w-full">
